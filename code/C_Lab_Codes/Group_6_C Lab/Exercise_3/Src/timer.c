...
void setTIM2callback(void (*cb)(void)) {
    TIM2HandlerFunction = cb;
    return;
}

void setTIM3callback(void (*cb)(void)) {
    TIM3HandlerFunction = cb;
    return;
}
...
void startOneShotTIM3(uint32_t ms, void (*cb)(void)) {
	/* Gate the clock and make sure everything is idle first */
	...
	TIM3HandlerFunction = cb;           // store callback
