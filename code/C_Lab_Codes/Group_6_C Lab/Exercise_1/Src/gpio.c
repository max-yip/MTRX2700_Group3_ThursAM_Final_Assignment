...
// Enable the button input on GPIOA pin 0
void enableGPIOAButton() {
    // Clear the two bits corresponding to pin 0 in the MODER register (sets it as input)
    GPIOA->MODER &= ~(0x3 << (0 * 2));
    return;
}

// Enable the LEDs on GPIOE pins 8 to 15
void enableGPIOELEDS() {
    // Get a pointer to the upper half (pins 8-15) of the GPIOE MODER register
    uint16_t *MODER = ((uint16_t *)&(GPIOE->MODER)) + 1;
    // Set pins 8-15 as general-purpose outputs (01b per pin, hence 0x5555)
    *MODER = 0x5555;
    return;
}
...
// Set the state of GPIOE LEDs (pins 8-15) using an 8-bit mask
void setLEDState(uint8_t led_mask) {
    // Get a pointer to the upper byte of the GPIOE ODR register (pins 8-15)
    uint8_t *ODR = ((uint8_t*)&(GPIOE->ODR)) + 1;
    // Write the mask to control the on/off state of each LED
    *ODR = led_mask;
    return;
}

// Get the current state of GPIOE LEDs (pins 8-15) as an 8-bit value
uint8_t getLEDState() {
    // Read the upper byte of the GPIOE ODR register
    uint8_t ODR = *(((uint8_t*)&(GPIOE->ODR)) + 1);
    return ODR;
}

void setButtonCallback(void (*callback)(void)) {
...
void init_main(void (*callback)(void)) {
	enableGPIOClocks();
	enableGPIOAButton();
	enableGPIOELEDS();
